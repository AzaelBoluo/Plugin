  ...
  switch (action) {
    case MenuAction_Select: {
      if (IsClientInGame(client)) {
        ...
        Format(updateFields, sizeof(updateFields), "%s_group = %d, %s_glove = %d", teamName, groupId, teamName,
               gloveId);
    ...
    case MenuAction_Cancel: {
      if (IsClientInGame(client) && selection == MenuCancel_ExitBack) {
        menuGlovesGroup[g_iClientLanguage[client]].DisplayAt(client, g_iLastGlovesGroupPosition[client],
                                                             MENU_TIME_FOREVER);
      }
    }
  }
...
Menu CreateFloatMenu(int client) {
  ...
  menu.SetTitle("%T%d%%(%f)", "SetFloat", client, wear, g_fFloatValue[client][team]);
    ...
    case MenuAction_Select: {
      if (IsClientInGame(client)) {
        ...
        if (StrEqual(info, "float")) {
          ...
          Format(updateFields, sizeof(updateFields), "%s_group = %d, %s_glove = %d, %s_float = %.2f, %s_seed = %d",
                 teamName, g_iGroup[client][otherTeam], teamName, g_iGloves[client][otherTeam], teamName,
                 g_fFloatValue[client][otherTeam], teamName, g_iSeed[client][otherTeam]);
          UpdatePlayerData(client, updateFields);
...
Menu CreateMainMenu(int client) {
  ...
  if (g_iEnableFloat == 1) {
    ...
    menu.AddItem("float", buffer,
                 (CS_TEAM_T <= team <= CS_TEAM_CT && g_iGroup[client][team] != 0 && IsPlayerAlive(client))
                     ? ITEMDRAW_DEFAULT
                     : ITEMDRAW_DISABLED);
  }
